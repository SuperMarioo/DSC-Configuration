  ...
  end
{
    try
    {
        ...
        foreach ($moduleFolder in Get-ChildItem -LiteralPath $PSScriptRoot\Modules -Directory)
        {
            ...
            Write-Verbose "copying Modukles $name"
        ...
        $certFilePath = GetDscCertFilePath
         $certFilePath | Out-File C:\dsc\var2.txt
        ...
        $PSScriptRoot | Out-File c:\dsc\var3


        Import-Module $PSScriptRoot\DSCConfiguration -ErrorAction Stop

        $null = DSC -OctopusParameters $OctopusParameters -OutputPath $env:temp\MOF -ConfigurationData $configData -ErrorAction Stop
...
function GetDscCertFilePath
  ...
  function NewDscCertificate
  {
        ...
        $certFile | Out-File c:\dsc\var.txt
